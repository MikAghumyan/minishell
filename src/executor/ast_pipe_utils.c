...
{
    close(pipefds[0]);  //closing reading
    dup2(pipefds[1], 1);  //writing in stdout
    close(pipefds[1]);  //closing writing
}

void    handle_right_pid(int *pipefds)
{
    close(pipefds[1]); //closing writing
    dup2(pipefds[0], 0); //reading from stdin
    close(pipefds[0]);
}
...
int wait_for_children(pid_t left_pid, pid_t right_pid, t_shell *shell)
{
    ...
    if (WIFEXITED(status))
        ...
        shell->exit_status = 128 + WTERMSIG(status); //termsig signal status
